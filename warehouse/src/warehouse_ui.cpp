...
class WarehouseUI {
 ...
 public:
  ...
  WarehouseUI() : display_(), memory_(WAREHOUSE_MEMORY_NAME), mutex_(WAREHOUSE_MUTEX_NAME){
    ...
    //===========================================================
    for(int i = 0; i < memory_->minfo.cols; i++){
      for(int j = 0; j < memory_->minfo.rows; j++){
        if(memory_->minfo.maze[i][j] == EXIT_CHAR)
        {
          exit_[COL_IDX] = i;
          exit_[ROW_IDX] = j;
        }
